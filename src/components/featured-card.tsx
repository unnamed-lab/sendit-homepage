...
export function FeaturedCard({ title, image, price, rating, reviews, href }: FeaturedCardProps) {
  return (
    <Link href={href}>
      <Card className="group relative aspect-[16/9] sm:aspect-[4/3] lg:aspect-[16/9] overflow-hidden bg-gray-900 border-gray-800">
        <Image
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/50 to-transparent p-4 sm:p-6 flex flex-col justify-end">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
            <div className="flex items-center">
              ...
              <span className="ml-2 text-xs sm:text-sm text-gray-400">{reviews} reviews</span>
            </div>
            <span className="text-base sm:text-lg font-bold text-white">{price} USDC</span>
          </div>
        </div>
      </Card>
    </Link>
